...
# -*-coding: utf-8 -*-
"""
本py文件按比例选取训练点，将不同类别的训练点分开作为训练字典
...
## Dev数据集debug模块
def dev(data_base, data_set):

	Residual = {}

	for test_l in range(data_base['label'].min() + 1, data_base['label'].max() + 1):
		# 选取测试集
		test = np.array(data_set['Dev' + str(test_l)]).T

		## 训练点分类
		for train_l in range(data_base['label'].min() + 1, data_base['label'].max() + 1):
			train = np.array(data_set['Train' + str(train_l)]).T
			JRC = Computing(test, train, 0.01)

			# print JRC.alpha
			# print('Done!')

			Residual['Residual of Train ' + str(train_l)] = JRC.residual
		...
		print('Dev' + str(test_l) + ' num = ' + str(len(fuck)))
		print('Dev' + str(test_l) + ' accuracy = ' + str(accuracy))
...
def main():
	...
	data_set = train_select_propatation(data_base, info_position_label, 0.3, 1)
